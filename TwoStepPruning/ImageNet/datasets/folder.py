...
class ImageFolder(data.Dataset):
    ...
    def __init__(self, data_path=None, transform=None, target_transform=None,
        ...
        if self.Train:
            ...
            cache_file='../datasets/train_key_cache'
        else:
            self.lmdb_dir = self.lmdb_dir+'/ilsvrc12_val_lmdb/'
            cache_file='../datasets/val_key_cache'
        ...
        # test the key
        key_index ='{:08}'.format(0)
        lmdb_cursor = self.lmdb_txn.cursor()
        value = lmdb_cursor.get(key_index.encode())
        if value == None:
            print("Need to reconfigure the key")
            self.key_reconfigure = True
