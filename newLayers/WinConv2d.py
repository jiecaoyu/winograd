...
class WinConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1):
        ...
        self.weight = nn.Parameter(torch.FloatTensor(out_channels, in_channels, kernel_size, kernel_size).normal_(0, 0.01))
    ...
    def forward(self, x):
        ...
        weight_t = self.weight.view(self.in_channels * self.out_channels,
                self.kernel_size, self.kernel_size)
        weight_t = torch.bmm(self.G.unsqueeze(0).expand(weight_t.size(0), *self.G.size()), weight_t)
        GT = self.G.transpose(0, 1)
        weight_t = torch.bmm(weight_t, GT.unsqueeze(0).expand(weight_t.size(0), *GT.size()))
        ...
        weight_t = weight_t.view(self.out_channels, self.in_channels, 8, 8).unsqueeze(0).unsqueeze(3).unsqueeze(4)
