...
class Winograd2d(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            y = torch.nn.functional.conv2d(input=x, weight=self.weight, bias=self.bias,
                    stride=1, padding=self.padding, groups=self.groups)
        else:
            # add padding
            if self.padding != 0:
    ...
    def regularize_grad(self):
        ...
        weight_t = torch.bmm(GT.unsqueeze(0).expand(weight_t.size(0), *GT.size()),
                weight_t)
        weight_t = torch.bmm(weight_t,
                self.G.unsqueeze(0).expand(weight_t.size(0), *self.G.size()))
